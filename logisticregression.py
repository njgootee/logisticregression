...
def gradient_descent(theta, X, Y, learn_rate=0.1, convergance=0.001):
    ...
    change_cost=1
    #For determining optimal learning rate
    cost_per_epoch = np.ones(shape=1000)
    ...
    while(change_cost > convergance):
        prev_cost=curr_cost
        #update cost per epoch array for (up to) first 1000 steps
        if i<=999:
            cost_per_epoch[i] = prev_cost
...
def fit(X, Y, learn_rate=0.01, convergance=0.01):
    ...
    theta, num_steps, cost_per_epoch = gradient_descent(theta, X, Y, learn_rate, convergance)
    return theta, num_steps, cost_per_epoch
